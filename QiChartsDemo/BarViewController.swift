...
class BarViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //添加刷新按钮
        refreshrBtn = UIButton.init(type: UIButton.ButtonType.custom)
        refreshrBtn.frame = CGRect.init(x: 30, y: 84, width: 40, height: 25)
        refreshrBtn.setTitle("刷新", for: UIControl.State.normal)
        refreshrBtn.backgroundColor = UIColor.red
        refreshrBtn.setTitleColor(UIColor.white, for: UIControl.State.normal)
    ...
    //添加柱状图
        func addBarChartView(){
            ...
            //刷新按钮响应
            refreshrBtn.addTarget(self, action: #selector(updataData), for: UIControl.Event.touchUpInside)
        }
